        ...
        <>
            { !_.isEmpty(calculateData.insuranceProvider) && (
                <Grid item onClick={()=> clickHadler('insurance')}>
                    ...
                    <Typography variant='h3' sx={{ color: (searchField && searchField!=='insurance') ? '#BBBBBB' : '#161616' }}>{calculateData?.insuranceProvider.name}</Typography>
                </Grid> 
            )}
            { !_.isEmpty(calculateData.insurancePlan) && (
                <Grid item mt="24px" onClick={()=> clickHadler('insurance')}>
                    ...
                    <Typography variant='h3' sx={{ color: (searchField && searchField!=='insurance') ? '#BBBBBB' : '#161616' }}>{calculateData?.insurancePlan.name}</Typography>
                </Grid>
            )}
            ...
            <Grid item mt="24px" onClick={()=> clickHadler('procedure')}>
                ...
                <Typography variant='h3' sx={{ color: (searchField && searchField!=='procedure') ? '#BBBBBB' : '#161616' }}>{calculateData?.procedure.billing_code}:{calculateData?.procedure.name}</Typography>
            </Grid>
            )}
            { ( (calculateData.location.zipCode && calculateData.location.locationType==='zipcode') || 
                ...
                <Grid item mt="24px" onClick={()=> clickHadler('location')}>
                    ...
                    <Typography variant='h3' sx={{ color: (searchField && searchField!=='location') ? '#BBBBBB' : '#161616' }}>
